...
def main(list_params, gpu_id, dataset_id, model_id, load_path, save_path, batch_size, learning_rate, n_epochs, augment_input, rotation, n_steps,
         ...
         emission_bias, clip_value, unique_emission, unique_glimpse, init_abv, scale_inputs, normalize_inputs, use_batch_norm, dropout,
    ...
    for i in range(reps):
        ...
        train(list_params=list_params, gpu_id=gpu_id, dataset_id=dataset_id, model_id=model_id, load_path=load_path,
             save_path=save_path_rep, batch_size=batch_size, learning_rate=learning_rate, n_epochs=n_epochs, augment_input=augment_input,
             rotation=rotation, n_steps=n_steps, glimpse_size= glimpse_size, coarse_size=coarse_size, fc_dim=fc_dim, enc_dim=enc_dim,
             dec_dim=dec_dim,n_classes=n_classes, output_mode=output_mode, use_init_matrix=use_init_matrix, headless=headless,
             emission_bias=emission_bias, clip_value=clip_value, unique_emission=unique_emission, unique_glimpse=unique_glimpse,
             scale_inputs=scale_inputs, normalize_inputs=normalize_inputs, use_batch_norm=use_batch_norm, dropout=dropout,
             use_weighted_loss=use_weighted_loss, localisation_cost_factor=localisation_cost_factor, zoom_factor=zoom_factor)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bn", "--use_batch_norm", type=int, default=1,
                        dest="use_batch_norm", help="Whether tlocalisation_o use batch normalization.")
