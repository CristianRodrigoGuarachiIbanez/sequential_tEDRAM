...
from typing import List, Tuple, Any, TypeVar, Dict, Union,Callable
...
class BilinearInterpolation(Layer):
    ...
    def __call__(self, inputs:List[Input]):
...
<<<<<<< HEAD
=======
        :param inputs: tensor None,10,120,160,1 or None,10,7,8,120,160,1 or None,10,7,120,160,1
        :param x: Tensor with shape None,26,26
        :param y: Tensor with shape None,26,26
        ...
        #print('INDICES ADVACE INDEXING:', indices, 'INPUTS:', inputs)

        indices = stack([b, y, x], 3) # (None,26,26,3), inputs -> (None,10,120,160,1)
        #print('INDICES ADVANCE INDEXING:', indices, 'INPUTS:', inputs)
    ...
    def grid_generator(self, batch:int) -> Tensor:
        ...
        #print('HOMOGENOUS COORDINATES:', homogenous_coordinates.shape)
    ...
    def interpolate(self, images:Input, homogenous_coordinates:Tensor, theta:Input) -> Tensor:
        ...
        with name_scope("Transformation"):
            #print(theta.shape, homogenous_coordinates.shape)
            theta = reshape(theta, shape=(shape(images)[0],2,3))
            ...
            #print("transformed: ", transformed, "homogenous: ", homogenous_coordinates, "theta: ", theta)
            x_transformed = transformed[:, :, :, 0] # shape=(None, 26, 26)
            y_transformed = transformed[:, :, :, 1]

            x = ((x_transformed + 1.) * cast(self.width, dtype=float32)) * 0.5 #shape=(None, 26, 26)
            y = ((y_transformed + 1.) * cast(self.height, dtype=float32)) * 0.5

        with name_scope("VariableCasting"):
            x0 = cast(math.floor(x), dtype=int32) #shape=(None, 26, 26)
            x1 = x0 + 1
            y0 = cast(math.floor(y), dtype=int32)
            y1 = y0 + 1


            x0 = clip_by_value(x0, 0, self.width - 1) #shape=(None, 26, 26),
            x1 = clip_by_value(x1, 0, self.width - 1)
            y0 = clip_by_value(y0, 0, self.height - 1) #shape=(None, 26, 26)
            y1 = clip_by_value(y1, 0, self.height - 1)
            x = clip_by_value(x, 0,cast(self.width, dtype=float32) - 1.0) #shape=(None, 26, 26),
            y = clip_by_value(y, 0, cast(self.height, dtype=float32) - 1)#shape=(None, 26, 26),
            #print("x1 : ", x1, "y1", y1)
        #print('x0:{},x1:{},y0{},y1:{},y:{},x:{}'.format(x0,x1,y0,y1,x,y))
        Ia: Tensor
        Ib: Tensor
        Ic: Tensor
        Id: Tensor
        with name_scope("AdvanceIndexing"):
            ...
            # images -> (None,10,120,160,1), x0 -> (None,26,26), y0 -> (None, 26,26)

            Ia = self.advance_indexing(images, x0, y0)
            Ib = self.advance_indexing(images, x0, y1)
            Ic = self.advance_indexing(images, x1, y0)
            Id = self.advance_indexing(images, x1, y1)
        #print('check Ia:{},Ib:{},Ic:{}, Id:{}'.format(Ia,Ib,Ic,Id))
        #print('IMAGE:', images)
        wa:Tensor
        wb:Tensor
        wc:Tensor
        ...
        #print('check wa:{}, Ia:{}, wb:{}, Ib:{}, wc{}, Ic:{}, wd:{},Id:{}'.format(wa,Ia,wb,Ib,wc,Ic,wd,Id))
