...
class BilinearInterpolation(Layer):
    ...
    def __init__(self, output_size, clip_max, **kwargs):
    ...
    # def call(self, tensors:Tuple[Input, Input], mask: Any =None):
    ...
    def call(self, tensors, mask=None):
        ...
        if self.clip_max < 1:
            ...
            t_clipped = K.stack((K.clip(t[:, 0], .0, self.clip_max), t[:, 1], t[:, 2], t[:, 3], K.clip(t[:, 4], .0, self.clip_max), t[:, 5]), axis=1)
            output = self._transform(X, t_clipped, self.output_size)
        else:

            output = self._transform(X, t, self.output_size) # (image_input, init_matrix_input, output)
        return output
    ...
    def _interpolate(self, image: Input, sampled_grids, output_size: Tuple[int,int]):
        ...
        x = K.cast(K.flatten(sampled_grids[:, 0:1, :]), dtype='float32') #sample_grid: (None, 3, 676) -> nur index 0 der zweiten Dim -> None, 1, 676
        y = K.cast(K.flatten(sampled_grids[:, 1:2, :]), dtype='float32')  # nur index 1 der zweiten Dim -> None, 1, 676 ->  None * 676 
        ...
        pixels_batch = K.arange(0, batch_size) * (height * width) # *(height * width) definiert die Step mit dem die Array Werte zunehmend ansteigen z.B. [0, 19200, 38400, 57600, ..], wenn  *(height * width) = 100
        ...
        base = K.repeat_elements(pixels_batch, flat_output_size, axis=1) # pixels_batch = (batch_size, 1) -> (batch_size, 1*flat_output_size) -> (10, 676)
    ...
    def _transform(self, X: Input, affine_transformation: Input, output_size: Tuple[int, int]):
        batch_size, num_channels = K.shape(X)[0], K.shape(X)[3]
        #print('BATCH SIZE FROM BINOCULAR',type(batch_size), 'NUMBER OF CHANNELS',type(num_channels))
        transformations = K.reshape(affine_transformation,
                                    shape=(batch_size, 2, 3)) # ? x  2 x 3 
        # transformations = K.cast(affine_transformation[:, 0:2, :], 'float32')
