...
@cython.cfunc
def tedram_model_56imgs(input_shape: Tuple, learning_rate: float = 0.0001, steps: int = 3, glimpse_size: Tuple[int, int] = (26, 26), coarse_size: Tuple[int, int] = (12, 12),
                 n_filters: int = 128, filter_sizes: Tuple[int, int] = (3, 5), n_features: int = 1024, RNN_size_1: int = 512, RNN_size_2: int = 512, n_classes: int = 4,
                 output_mode: int = 0, use_init_matrix: int = True, emission_bias: int = 1, clip_value: int = 1, unique_emission: int = False,
                 unique_glimpse: int = False, bn: bool = True, dropout:int = 0, use_weighted_loss: int = False, localisation_cost_factor: float = 1.0) -> Model:
    """
        ...
        Parameters:
            * input_shape: input image dimensions (N,10,56,120,160)
            * learning_rate: ~
    ...
    for i in range(0, steps): #5
        print("index", i,' CALL TEDRAM CELL:', input_shape[1:], "LEN: ", len(edram_cell))
        edram_cell.append(tedram_cell(input_shape=input_shape[1:], glimpse_size=glimpse_size, n_filters=n_filters, RNN_size_1=RNN_size_1, RNN_size_2=RNN_size_2, bn=bn,
                                              dropout=dropout, layers=layers, output_localisation=output_localisation, step=str(i), unique_emission=unique_emission,
    ...
    print('INPUT IMAGE COARSE:', input_image_co.shape)
    ...
    if unique_emission == False:
        ...
        init_matrix = Dense(6, activation='tanh', weights=emission_weights(RNN_size_2, emission_bias[0]),
                            name='emission_loc')(x)
    ...
    for i in range(steps):
        ...
        input_image_i = sl_i(input_image)
        step.append(edram_cell[i](
            [input_image_i, step[i][1], step[i][2], step[i][3], step[i][4], step[i][5], b26, b24, b12,
             b6 if glimpse_size == (26, 26) else b4, b4]))

    print("LEN :", len(step), len(step[1]), step)
    ########################
    ###  Define Outputs  ###
    ########################
    if output_mode == 0:
        ...
        if steps == 1:
            ...
            classifications = Reshape((steps, n_classes), name='classifications')(
                concatenate([step[i][0] for i in range(1, steps+ 1)]))
            localisations = Reshape((steps + 1, 6), name='localisations')(
                concatenate([step[i][1] for i in range(0, steps+ 1)]))
    outputs: List[Any] = [classifications, localisations]

    # print(classifications.shape) # (None, 6)
    # print(localisations.shape) # (None, 2, 6)
    # ------- build the model
    # print(len(inputs), len(outputs)) # 11 2
